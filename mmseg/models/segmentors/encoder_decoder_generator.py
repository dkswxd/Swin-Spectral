...
class EncoderDecoderGenerator(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None,
                 generator_file='data/HSI/13000_EMA.pth'):
        super(EncoderDecoderGenerator, self).__init__(init_cfg)
        ...
        self.fake_rate = 1 - 1 / (1 + fake_rate)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        with torch.no_grad():
            self.generator.eval()
            bs, _, h, w = gt_semantic_seg.shape
            nc = self.decode_head.num_classes
            seg = torch.cuda.FloatTensor(bs, nc, h, w).zero_()
            ...
            img_gen = F.instance_norm(img_gen)
            img = img * rand + img_gen * (1 - rand)

        '''
        import numpy as np
        import cv2
        i = img_gen.cpu().detach().numpy()
        i = i[0,6:30:10,:,:]
        i = np.transpose(i,(1,2,0))
        i-=np.mean(i,axis=(0,1))
        i/=np.std(i,axis=(0,1))
        i*=16
        i+=128
        ii = i.astype(np.uint8)
        cv2.imwrite('a.png',ii)
        '''
        x = self.extract_feat(img)
